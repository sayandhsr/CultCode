...
fun IdeScreen(initialLanguage: String, onNavigate: (NavKey) -> Unit) {
    ...
    Scaffold(
        ...
        if (isJupyterMode) {
            LazyColumn(
                modifier = Modifier.fillMaxSize().padding(padding).padding(Space.md),
                ...
                item { Spacer(modifier = Modifier.height(Space.md)) }
            }
        } else {
            Column(modifier = Modifier.fillMaxSize().padding(padding).padding(Space.md)) {
                OutlinedTextField(
                    value = vsCodeText,
